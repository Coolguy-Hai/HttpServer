...
private:
    static const int MAX_PROCESS_NUMBER = 8;   //最大进程数量
    static const int USER_PRE_PROCESS = 65535;  //每个子进程最多能处理的客户数量
    ...
    while(!m_stop){
        ...
        for(int i = 0;i < number;i++){
            ...
            if((sockfd == pipefd) && (events[i].events & EPOLLIN)){
                ...
                //printf("run_child...connfd\n");
                users[sockfd].process();
                close(sockfd);
            }
            else{
                //printf("run_child...nothing\n");
                continue;
            }
        }
    }
